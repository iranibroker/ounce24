...
export class SignalsService {
  constructor(@InjectModel(Signal.name) private signalModel: Model<Signal>) {
    setTimeout(() => {
      this.findAll().then(all => {console.log(all)})
    }, 3000);
  }

  async create(dto: Pick<Signal, 'type' | 'entryPrice' | 'maxPrice' | 'minPrice'>): Promise<Signal> {
    const createdCat = new this.signalModel(dto);
